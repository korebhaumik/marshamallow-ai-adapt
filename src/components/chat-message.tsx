...
export function ChatMessage({ message, isLoading, isLast, ...props }: ChatMessageProps) {
	return (
		<>
			<div
			...
			>
				<div
					className={`flex h-8 w-8 shrink-0 select-none items-center justify-center rounded-lg border shadow ${
						message.role === "user" ? "bg-white text-black border-stone-50" : "bg-black"
					}`}
				>
				...
				<div className="ml-4 flex-1 space-y-2 overflow-hidden px-1">
					<MemoizedReactMarkdown
						className="prose prose-headings:text-black/80 prose-strong:text-black prose-invert text-black/75 break-words dark:prose-invert prose-p:leading-relaxed prose-pre:p-0"
					...
					>
						{message.role === "user" ? JSON.parse(message.content).input : message.content}
					</MemoizedReactMarkdown>
				</div>
			</div>
			{/* {message.role === "user" && (
