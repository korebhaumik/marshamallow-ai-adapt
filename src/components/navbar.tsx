...
export default function Navbar({}: Props) {
	...
	return (
		...
		<header
			className="sticky top-0 z-50 flex items-center justify-between w-full h-16 px-6 border-b  border-stone-200"
			style={{
		...
		>
			<div className="flex items-center justify-between w-full">
				<div className="flex items-center">
					...
					{user ? <User /> : <GithubLogin handleSignInGithub={handleSignInGithub} />}
					{/* <GithubLogin handleSignInGithub={handleSignInGithub}/> */}
					{/* <User /> */}
				</div>
				<div>
					{/* <button className="text-sm px-4 py-2 border rounded">Sign in</button> */}
					{user && <Plus />}
				</div>
			</div>
		</header>
...
function GithubLogin({ handleSignInGithub }: { handleSignInGithub: () => void }) {
	return (
		<Link
			href="https://github.com/korebhaumik/adapt-ai"
			className="text-sm px-4 py-1 flex items-center border-zinc-700 text-white bg-black border rounded"
		>
			<svg
...
function User() {
	...
	return (
		<div
			ref={elementRef}
			className="w-8 h-8 relative border text-sm text-center border-zinc-800 rounded-full cursor-pointer"
		>
			<span onClick={() => setIsOpen(!isOpen)} className="mt-2 text-xs text-zinc-300 block">
				{initials}
			</span>
...
function Dropdown({ setIsOpen }: { setIsOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
	// const { signOut, user } = useAuth();
	//@ts-expect-error
		...
		<>
			{!isPlusUser ? (
				<button
					...
					className="text-sm px-4 py-1 border ml-2 text-black bg-slate-50 font-medium rounded"
				>
					Buy GPT-Plus
				</button>
			) : (
				<button
					disabled
					className="text-sm px-4 py-1 flex items-center border ml-2 text-black bg-slate-50 font-medium rounded"
				>
					<span>GPT-Plus</span>
					<TickSVG />
				</button>
...
function LogoSVG() {
	return (
		<svg
			...
			viewBox="0 0 24 24"
			fill="none"
			className="text-[#1D4ED8] border-[1px] border-[#1D4ED8]/50 rounded-xl w-9 h-9 p-[7px] shadow-sm bg-[#DEEAFC]"
		>
			<path
